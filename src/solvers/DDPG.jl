...
function POMDPs.action{S,A}(p::DDPGPolicy{S,A}, s::S, a::A=create_action(p.mdp) )
    ...
    s_vec = vec(p.mdp, s)
    mx.copy!( p.exec.arg_dict[p.input_name], s_vec )

    mx.forward( p.exec )

    mx.copy!(p.a_vec, p.exec.outputs[1])

    return Base.convert(A, vec(p.a_vec))#devec(mdp, vec(p.a_vec))
...
function action{S,A}(p::ExplorationPolicy, actor::NeuralNetwork, mdp::MDP{S,A}, s, rng, a::A=create_action(mdp)) # exploration
    ...
    s_vec = vec(mdp, s)
    mx.copy!( get(actor.exec).arg_dict[actor.input_name], s_vec )

    mx.forward( get(actor.exec) )

    a_vec = mx.copy!(zeros(Float32, size(get(actor.exec).outputs[1])), get(actor.exec).outputs[1])

    noise = next!(p, rng)

    a_vec += noise

    return convert(A, vec(a_vec)), s_vec, vec(a_vec) #devec(mdp, vec(a_vec)), s_vec, a_vec
end


function ddpg_update!(solver::DDPG, disc::Real, rng::AbstractRNG, input_idx::Int)

    actor, critic, actor_target, critic_target = solver.actor, solver.critic, solver.actor_target, solver.critic_target
    mem = solver.replay_mem
    target_refresh_rate = solver.target_refresh_rate
    ...
    for idx = 1:actor.batch_size
        ...
        mx.copy!( get(critic.exec).arg_dict[critic.input_name[MDPState]], state(mem, sp_idx) )
        mx.copy!( get(critic.exec).arg_dict[critic.input_name[MDPAction]], action(mem, a_idx) )
        mx.forward( get(critic.exec) )
        
        q = mx.copy!( zeros(Float32, size(get(critic.exec).outputs[1])), get(critic.exec).outputs[1])[1]
        ...
        # backprop on critic
        mx.backward( get(critic.exec), grad_out )

        # accumulate these gradients elsewhere
        for (i,param) in enumerate(get(critic.exec).grad_arrays)
            @mx.inplace critic.grad_arrays[i] += param
        end

        # backprop on critic w/ ones as grad_out (use mx.gradient(actions?))
        mx.backward( get(critic.exec), mx.ones(1,1) )

        # forward on actor (for backprop)
        mx.forward( get(actor.exec) )

        # backprop on actor w/ prev as grad_out
        mx.backward( get(actor.exec), get(critic.exec).grad_arrays[input_idx]) 
    end
    ...
    for (param, param_target) in zip( get(actor.exec).arg_arrays, actor_target.arg_arrays )
        @mx.inplace param_target .*= (1.-target_refresh_rate)
        @mx.inplace param_target .+= target_refresh_rate * param
    end
...
function solve(solver::DDPG, mdp::MDP, policy::DDPGPolicy=POMDPs.create_policy(solver, mdp), rng::AbstractRNG=RandomDevice())
    ...
    for ep = 1:solver.num_epochs
        ...
        try
            while !isterminal(mdp, s) && step <= max_steps
                ...
                # TODO prune return args
                ap, sp_vec, ap_vec = action(solver.exp_pol, solver.actor, mdp, sp, rng) # convenience, maybe remove ap_idx, s_vec
    ...
    # return policy
    # TODO update policy more frequently
    # TODO make new exec that doesn't need to train
    policy.exec = get(solver.actor.exec)
    return policy
end
